...
def NeuralNetTrainer(filename: String, iterations: Integer) : Array[Double] = {
	...
	var layer2 = Array.fill[Double](targets.length)(2*rand.nextDouble-1); //This layer will be applied to outputs obtained from layer1
		...
		var out2 = matmultiply(out1, Array(layer2)).map{_.map{x=> sigmoid(x,false)}}; //These map onto our actual predictions

		//Final layer prediction error
		var error2 = targets.zip(out2(0)).map{t: (Double,Double) => t._1 - t._2}; 

		//Back-propagation time!

		//Elementwise multiplication of error and 'confidence' of a weight
		var d_layer2 = out2(0).map{x=> sigmoid(x,true)}.zip(error2).map{t:(Double,Double) => t._1*t._2}

		var error1 = matmultiply(d_layer2,T(layer1))		
